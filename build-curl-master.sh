...
if ! git clone https://github.com/curl/curl.git "$CURL_DIR"
then
...
cd "$CURL_DIR"
git checkout "$CURL_TAG" &>/dev/null

if false; then

if [[ "$IS_OLD_DARWIN" -ne 0 ]]
then
    cp ../patch/curl-darwin.patch .
    patch -u -p0 < curl-darwin.patch
    echo ""
fi
...
if [[ ! -f ./configure ]]
then
    echo "Failed to autoreconf CURL"
    [[ "$0" = "${BASH_SOURCE[0]}" ]] && exit 1 || return 1
fi
